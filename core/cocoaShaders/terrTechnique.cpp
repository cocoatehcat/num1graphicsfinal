...
#include "../external/ogldev_util.h"
...
bool TerrainTechnique::Init()
{
    ...
    if (!AddShader(GL_VERTEX_SHADER, "terrain.vert")) {
        return false;
    }

    if (!AddShader(GL_FRAGMENT_SHADER, "terrain.frag")) {
        return false;
    }
    ...
    m_VPLoc = GetUniformLocation("viewMatrix");
